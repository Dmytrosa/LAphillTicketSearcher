...
const performancesService_1 = require("./performancesService");
const pricesService_1 = require("./pricesService");
const zonesService_1 = require("./zonesService");
class SeatsService {
    getSeatsInfo(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const performanceId = yield (0, performancesService_1.getPerformanceId)(id, miniState_1.sourceId); // Виклик зовнішньої функції
            if (!performanceId) {
                throw new Error('Some error occurred. Performance not found. Try another event');
            }
            const zoneInfo = yield (0, zonesService_1.getZoneInfo)(performanceId); // Виклик зовнішньої функції
            if (!zoneInfo) {
                throw new Error('Information about zones not found.');
            }
            const priceInfo = yield (0, pricesService_1.getPriceInfo)(id, miniState_1.sourceId); // Виклик зовнішньої функції
            if (!priceInfo) {
